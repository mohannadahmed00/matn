...
/**
 * Owns the Notes tab state (US2 FR-012/FR-020; US3 FR-017 adds the notes list, this task).
 * Entry taps emit a one-shot `(matnId, verseId)` navigation pair via [navigation] (same idiom as
...
class NotesTabViewModel(
    ...
    fun onNoteClick(entry: NoteEntry) {
        viewModelScope.launch { _navigation.send(entry.ref.matnId to entry.ref.verseId) }
    }
}
