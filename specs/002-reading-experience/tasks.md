...
- [X] T001 Add navigation dependency: in `gradle/libs.versions.toml` add version
...
- [X] T002 [P] Add the bundled Arabic reading font: place `Amiri-Regular.ttf` and
...
- [X] T003 [P] Create interface-chrome string resources (FR-021). Create
...
- [X] T004 [P] Create the use-case base contracts in `core/usecase/UseCase.kt`:
...
- [X] T005 [P] Create `domain/model/ReadingFontSize.kt`:
  `enum class ReadingFontSize { SMALL, MEDIUM, LARGE, XLARGE; companion object { val DEFAULT = MEDIUM } }`.
- [X] T006 [P] Create the derived-read domain models: `domain/model/MatnSummary.kt`
...
- [X] T007 Create `presentation/base/BaseViewModel.kt`:
...
- [X] T008 Create the theme in `presentation/theme/`: `Type.kt` builds the Amiri
...
- [X] T009 [P] Create `presentation/common/DurationFormatter.kt`: pure function
  `fun formatDuration(ms: Long): String` → `m:ss` (and `h:mm:ss` when ≥ 1h). No platform APIs.
- [X] T010 [P] Create `presentation/common/CoverImage.kt`: a composable
...
- [X] T011 Create the navigation shell `presentation/navigation/MatnNavHost.kt`: a `NavHost`
...
- [X] T012 Replace the body of `App.kt` so it renders `MatnTheme { MatnNavHost() }` (remove the
...
- [X] T013 [P] [US1] Create `domain/usecase/ObserveVersesUseCase.kt`:
...
- [X] T014 [P] [US1] Create `domain/usecase/GetMatnDetailsUseCase.kt`:
...
- [X] T015 [US1] Create `presentation/details/MatnDetailsUiState.kt` with `MatnDetailsUiState`,
...
- [X] T016 [US1] Create `presentation/details/MatnDetailsViewModel.kt` extending
...
- [X] T017 [US1] Create `presentation/details/MatnDetailsScreen.kt`: a `Column` with the header
...
- [X] T018 [US1] Wire it up: in `di/ContentModule.kt` add
...
- [X] T019 [P] [US1] Create `shared/src/commonTest/kotlin/com/giraffe/matn/presentation/MatnDetailsViewModelTest.kt`
...
- [X] T020 [US2] Add derived-total reads to
...
- [X] T021 [US2] Extend the library read: add
...
- [X] T022 [P] [US2] Create `domain/usecase/ObserveLibraryUseCase.kt`:
...
- [X] T023 [US2] Create `presentation/home/HomeUiState.kt` per [data-model.md](./data-model.md)
  §4.1 (`isLoading`, `items: List<MatnSummary>`, `isEmpty`, `error`).
- [X] T024 [US2] Create `presentation/home/HomeViewModel.kt` extending
...
- [X] T025 [US2] Create `presentation/home/HomeScreen.kt`: `LazyVerticalGrid` (2 columns) of
...
- [X] T026 [US2] Wire it: in `di/ContentModule.kt` add `factory { ObserveLibraryUseCase(get()) }`.
...
- [X] T027 [P] [US2] Create `.../commonTest/.../presentation/HomeViewModelTest.kt` with a fake
...
- [X] T028 [US3] In `MatnDetailsViewModel` build the `chapters: List<ChapterRow>` for the state:
...
- [X] T029 [US3] Create `presentation/details/TableOfContents.kt`: a composable listing
...
- [X] T030 [US3] Implement scroll-to in `MatnDetailsScreen`: hold a `rememberLazyListState()`;
...
- [X] T031 [P] [US3] Extend `MatnDetailsViewModelTest`: structured fixture →
...
- [X] T032 [US4] Add the settings store to `Content.sq`: `CREATE TABLE app_setting (key TEXT NOT
...
- [X] T033 [US4] Create `domain/repository/ReadingPreferencesRepository.kt`:
  `fun observeFontSize(): Flow<ReadingFontSize>` and
  `suspend fun setFontSize(size: ReadingFontSize): Resource<Unit>`.
- [X] T034 [US4] Create `data/repository/ReadingPreferencesRepositoryImpl.kt` over
  ...
  Principle VI).
- [X] T035 [P] [US4] Create `domain/usecase/GetFontSizeUseCase.kt`
...
- [X] T036 [US4] Extend `MatnDetailsViewModel`: take `GetFontSizeUseCase`/`SetFontSizeUseCase`;
...
- [X] T037 [US4] Add the font-size control to `MatnDetailsScreen`: e.g. a top-bar action opening
...
- [X] T038 [US4] Wire DI: in `di/ContentModule.kt` add
...
- [X] T039 [P] [US4] Create `.../commonTest/.../data/ReadingPreferencesRepositoryTest.kt` (or
...
- [X] T040 [P] Localization sweep: confirm every user-facing string comes from the string
...
- [X] T041 [P] Performance check: confirm the verse `LazyColumn` and library `LazyVerticalGrid`
...
- [X] T042 Scope guard: verify no FR-020 out-of-scope element leaked in (no play/pause,
...
- [X] T043 Run the full [quickstart.md](./quickstart.md) validation: Section A (`commonTest`
