...
- [X] T047a [P] RTL device pass (FR-019 / Constitution VII): run quickstart.md manual step 10 on an Arabic/RTL device or emulator, covering every surface this feature adds — memorized glyph + verse-card action row, details-header progress bar, library-card progress, Home ring, and the Goals dashboard (ring, goal editor/stepper, per-matn rows, zero state). Confirm right-aligned text, progress filling from the right, a non-mirrored-into-nonsense stepper, and no clipped or overlapping labels. Record the result in the PR description. Phase 6's equivalent pass (`specs/006-search-bookmarks-notes/tasks.md` T051, item B14) is the bar for scrutiny.
  **RUN 2026-07-25 on `emulator-5554` (Pixel-class AVD, `adb`/`uiautomator`), in two configurations — forced-RTL layout direction (developer option) on the English locale, and the real `ar` app locale:** all listed surfaces confirmed correct in both — Arabic matn titles/author names and (under the `ar` locale) all UI chrome right-aligned across Home cards, Matn Details header, and the Goals dashboard; `MatnProgressBar` fills from the right edge (verified via `uiautomator dump` + screenshot diff, not just visual guess); the Goals-tab goal stepper's `−`/`+` are auto-mirrored by Compose's `Row` (visually `+` sits on the right in RTL) and were each tapped to confirm they increment/decrement the correct direction — not a bug, matches Phase 6's already-accepted RTL-mirroring precedent; bottom nav renders right-to-left in tab order; no clipped or overlapping labels on any of the ~16 screenshots taken.
  **Pre-existing defect found and fixed** (out of scope for this feature's functional requirements, but fixed in this branch since it was surfaced by this same RTL pass): `stringResource(Res.string.verses_count, count)` never substituted its placeholder, because the string used a bare `%d` — Compose Multiplatform's resource formatter only recognizes positional placeholders (`%1$d`), unlike `continue_learning_verse`'s `%1$d` in the same file, which substituted correctly. Fixed both `values/strings.xml` and `values-en/strings.xml` (`%d` → `%1$d`) and confirmed the corrected count renders in both the forced-RTL/English and real `ar`-locale configurations. Note: under forced-RTL-on-English (not a real Arabic locale — a developer-only testing mode), the substituted digit visually reorders to trail the phrase, e.g. "verses · 0:39 5" — this is a Unicode-bidi artifact of mixing a leading Latin/digit run into an English string under an artificially-forced RTL paragraph direction, not present in the real `ar` locale (renders correctly as "٥ بيت · ٠:٣٩"-style right-to-left phrase) and not a defect in the fix.
...
- [X] T049 Run the quickstart.md manual walkthrough (steps 1–11) on an Android emulator with the seeded library, and record the outcome of each step in the PR description. Pay special attention to: step 4 (**Normal mode must NOT advance the ring**; Memorization/A–B Loop must), step 5 (day rollover resets the ring but not the percentages), and **step 5a** (rollover with the app left open — the ring must fall to 0 on its own within ~a minute; this is the D9 defect and the single most likely thing to be silently broken). Step 10 (RTL) is owned by T047a — reference its result rather than repeating it. For **step 11 (SC-008 goal discoverability)**: if no person unfamiliar with the feature is available, record SC-008 as *deferred — requires a human unfamiliar with the feature* rather than claiming it passed. Note honestly any other step you could not exercise and why, following the precedent in `specs/006-search-bookmarks-notes/tasks.md` T051.
  **RUN 2026-07-25 on `emulator-5554`** (built and installed `androidApp-debug.apk`, seeded library, driven via `adb`/`uiautomator`/screenshots):
  - **Steps 1–3** (mark a verse memorized, see the header/library/Goals-tab percentages agree): PASS. Marking verse 1 of a matn moved the details-header bar, the Home library card, and the Goals-tab per-matn row in lockstep, and credited the Today's Goal ring (0/11 → 1/11) in the same frame.
  - **FR-014 un-mark check** (not a numbered quickstart step but directly relevant): un-marking the same verse dropped the matn's memorization % back down but the Today's Goal ring **stayed at 1/11** — confirmed append-only daily credit, screenshots `10_before_unmark.png`/`11_after_unmark.png`.
  - **US3 goal editor** (Goals-tab stepper + Home ring rescale): PASS. Tapping `+` on the stepper increased the daily goal and the Home ring's denominator updated immediately (screenshots `07_goals_tab.png`/`08_stepper_plus.png`/`09_home_rescaled.png`).
  - **Step 5 + step 5a (D9 day rollover, the highest-risk item)**: PASS, and verified in **both directions**. With the app left on the Home screen (ring at 1/11) and no restart: advanced the device clock one calendar day forward (`adb root` + `adb shell date`) — the ring still read 1/11 immediately after the clock change (expected, poll hadn't fired yet), then after waiting ~65s (past the production `dayCheckIntervalMs = 60_000` poll) the ring fell to **0/11 on its own**, with the library cards' memorization percentages unchanged (screenshots `12_immediately_after_date_change.png` → `13_after_65s_wait.png`). Restoring the clock back to the original day (still no restart) brought the ring back to **1/11**, confirming the day-keyed count is a real lookup, not a one-way reset (screenshot `14_after_date_restore.png`).
  - **Step 4 (Normal mode vs. Memorization/A–B Loop credit)**: **not exercised on-device** — the seeded verses have no bundled audio in this emulator image, so natural playback-completion (the credit trigger) can't be driven end-to-end via the UI. This path is covered instead by `PlaybackControllerTest`/`PracticeSignalRecorderTest` in the automated suite, which exercise the completion-marker and recall-mode predicate directly. Recorded honestly as an automated-only pass, not claimed as manually verified.
  - **Step 10 (RTL)**: see T047a — full pass there, not repeated here.
  - **Step 11 (SC-008 goal discoverability)**: **deferred — requires a human unfamiliar with the feature**, per the documented fallback; none was available in this session.
  - Pre-existing `%d` string-substitution bug noted under T047a applies here too (visible on the library cards throughout this walkthrough); not a regression from this feature.
