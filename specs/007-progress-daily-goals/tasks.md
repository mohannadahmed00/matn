...
- [X] T047a [P] RTL device pass (FR-019 / Constitution VII): run quickstart.md manual step 10 on an Arabic/RTL device or emulator, covering every surface this feature adds — memorized glyph + verse-card action row, details-header progress bar, library-card progress, Home ring, and the Goals dashboard (ring, goal editor/stepper, per-matn rows, zero state). Confirm right-aligned text, progress filling from the right, a non-mirrored-into-nonsense stepper, and no clipped or overlapping labels. Record the result in the PR description. Phase 6's equivalent pass (`specs/006-search-bookmarks-notes/tasks.md` T051, item B14) is the bar for scrutiny.
  **RUN 2026-07-25 on `emulator-5554` (Pixel-class AVD, device locale Arabic/RTL, `adb`/`uiautomator`):** all listed surfaces confirmed correct — Arabic matn titles and author names right-aligned across Home cards, Matn Details header, and the Goals dashboard; `MatnProgressBar` fills from the right edge (verified via `uiautomator dump` + screenshot diff, not just visual guess); the Goals-tab goal stepper's `−`/`+` are auto-mirrored by Compose's `Row` (visually `+` sits on the right in RTL) and were each tapped to confirm they increment/decrement the correct direction — not a bug, matches Phase 6's already-accepted RTL-mirroring precedent; bottom nav (Library/Goals/Notes/Settings) renders right-to-left in tab order; no clipped or overlapping labels observed on any of the ~14 screenshots taken. **One pre-existing defect found, out of scope for this feature**: `stringResource(Res.string.verses_count, count)` does not substitute the `%d` placeholder on this Compose Multiplatform version — library cards render the literal unsubstituted string (visually reordered by RTL bidi to read like "d verses · 0:39%"). Confirmed via accessibility-tree dump showing the raw unsubstituted text. This bug predates Phase 7 (the string and its call site are unchanged by this feature) and affects every screen that shows a verse count, not just the new ones — flagged for a separate fix, not blocking this PR.
